...
use crate::draw_components::{BoxDrawComponent, DrawComponent, NullComponent, VSplitDrawComponent};
...
impl Gear for ScoreEditorGear {
    fn get_draw_component(&self) -> Box<dyn DrawComponent> {
        ...
        Box::new(BoxDrawComponent::new(Box::new(VSplitDrawComponent::new(
    ...
    fn handle_event(&mut self, event: &InputEvent) -> bool {
        ...
        match event {
            ...
            InputEvent::ViewerBarNext => {
                ...
                if let Some(viewport_result) = self.viewport_draw_result {
                    ...
                    self.score_viewport = self
                        .score_viewport
                        .set_time_point(self.score_viewport.time_point + 32);
                }
                true
            }
            ...
            InputEvent::CursorLeft => {
                ...
                if let Some(viewport_result) = self.viewport_draw_result {
                    if self.cursor.time_point() <= viewport_result.time_point_start + 32
                        && self.score_viewport.time_point >= 32
                    {
            ...
            InputEvent::InsertNote => {
                match self.cursor.mode() {
                    CursorMode::Select(_start, _end) => {
                        ...
                        let duration = selection_range.time_point_end_b32
                        ...
                        let existing_notes = self
                            ...
                            .lock()
                            .unwrap()
                            .notes_starting_at_time(time_point, None)
                            .into_iter()
            ...
            InputEvent::Yank => {
                if let CursorMode::Select(_, _) = self.cursor.mode() {
                    ...
                    let selection_score = self
                        ...
                        .lock()
                        .unwrap()
                        .clone_at_selection(selection_range, None); // No instrument filter
                    self.cursor = self
                        .cursor
                        .yank()
                        .right(self.score_viewport.resolution.duration_b32());
            ...
            InputEvent::Cut => {
                if let CursorMode::Select(_, _) = self.cursor.mode() {
                    ...
                    self.score
                        .lock()
                        .unwrap()
                        .delete_in_selection(selection_range, None); // No instrument filter
                    self.cursor = self.cursor.end_select();
                    self.selection_buffer = SelectionBuffer::Score(
                        selection_score.translate(Some(self.cursor.time_point())),
                    );
...
mod tests {
    ...
    fn test_note_insertion() {
        ...
        assert!(gear.handle_event(&InputEvent::InsertNote));
        {
            ...
            assert!(
                notes.is_empty(),
                "Note should be removed after second insertion"
            );
        }
    }
