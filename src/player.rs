use crate::instruments::{self, Instrument, InstrumentReference, InstrumentRegistry};
...
impl Player {
    ...
    pub fn instruments(&self) -> Vec<InstrumentReference> {
        self.instrument_registry
            .get_all_instruments()
            .iter()
            .map(|instrument| instrument.to_instrument_reference())
            .collect()
    }
        ...
        if let Some(end_tick) = note_env.end_tick {
            ...
            if current_tick >= end_tick {
                let release_progress =
                    current_tick.saturating_sub(end_tick) as f64 / release_ticks as f64;
    ...
    fn update_active_notes(&mut self) {
        ...
        for note_env in &mut self.active_notes {
            ...
            if note_env.note.onset_b32 + note_env.note.duration_b32 <= time_b32
                && note_env.end_tick.is_none()
            {
    ...
    fn handle_time_update(&mut self) {
        ...
        if self.loop_state.is_looping() {
            if let (Some(start), Some(end)) =
                (self.loop_state.start_time_b32, self.loop_state.end_time_b32)
            {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        for note_env in &self.active_notes {
            ...
            if let Some(instrument) = self
                .instrument_registry
                .get_instrument(note_env.note.instrument_id)
            {
                // Generate sample using the instrument and apply envelope
                total_amplitudes +=
                    instrument.generate_sample(note_env.note.pitch, self.tick, self.sample_rate)
                        * envelope_value;
            } else {
