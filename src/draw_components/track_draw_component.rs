...
impl DrawComponent for TrackDrawComponent {
    fn draw(&self, buffer: &mut Vec<Vec<char>>, pos: &Position) -> Vec<DrawResult> {
        ...
        self.draw_pitches(buffer, pos, &pitches);
...
impl TrackDrawComponent {
    ...
    fn draw_track(&self, buffer: &mut Vec<Vec<char>>, pos: &Position) -> ViewportDrawResult {
        ...
        self.draw_grid(buffer, pos, &pitches);

        // Draw playhead and loop markers
        let time_point = self.base.draw_playhead(buffer, pos, pitches.len());

        // Draw notes for this specific instrument
        self.draw_track_notes(buffer, pos, &pitches);

        // Draw the cursor
        self.draw_cursor(buffer, pos, &pitches);

        ViewportDrawResult {
            pitch_high: pitches
                .first()
                .cloned()
                .unwrap_or(self.base.score_viewport.middle_pitch),
            pitch_low: pitches
                .last()
                .cloned()
                .unwrap_or(self.base.score_viewport.middle_pitch),
    ...
    pub fn draw_cursor(&self, buffer: &mut Vec<Vec<char>>, pos: &Position, pitches: &Vec<Pitch>) {
        ...
        for col in 0..pos.w - 1 {
            for (row, pitch) in pitches.iter().enumerate() {
                if self.base.cursor.visible_at(*pitch, time_point) {
        ...
        for col in 0..pos.w - 1 {
            ...
            for _ in 0..self.base.score_viewport.resolution.duration_b32() {
                ...
                let active_notes = self
                    ...
                    .lock()
                    .unwrap()
                    .notes_active_at_time(time_point, Some(self.instrument_id));
                    ...
                    self.base.selection_buffer
                {
                    ...
                    let selected_notes = selection_buffer_score
                        .notes_active_at_time(time_point, Some(self.instrument_id));
...
mod tests {
    ...
    fn create_test_track_component(instrument_id: u32) -> TrackDrawComponent {
        ...
        let score_viewport =
            ScoreViewport::new(Pitch::new(Tone::C, 4), Resolution::Time1_16, 32, 0);
    ...
    fn test_draw_pitches() {
        ...
        let pitches = component.visible_pitches(&pos);
        component.draw_pitches(&mut buffer, &pos, &pitches);
