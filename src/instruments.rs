...
pub trait Instrument: Send + Sync {
    ...
    fn to_instrument_reference(&self) -> InstrumentReference {
        InstrumentReference {
            id: self.id(),
...
impl Instrument for FMSynthesizer {
    fn generate_sample(&self, pitch: Pitch, time_ticks: u64, sample_rate: u64) -> f64 {
        ...
        let feedback_amount = self.feedback
            * (2.0 * PI * carrier_freq * ((time_ticks - 1) as f64) / (sample_rate as f64)).sin();
